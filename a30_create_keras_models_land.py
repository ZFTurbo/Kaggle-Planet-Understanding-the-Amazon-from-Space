...
def train_single_model(num_fold, cnn_type, train_files, valid_files, train_labels, valid_labels):
    ...
    model = get_pretrained_model(cnn_type, 13, final_layer_activation='sigmoid')
    ...
    filename = HISTORY_FOLDER_PATH + 'history_{}_{}_{:.4f}_lr_{}_{}_land.csv'.format(cnn_type, num_fold, min_loss, get_learning_rate(cnn_type), now.strftime("%Y-%m-%d-%H-%M"))
...
def run_cross_validation_create_models(nfolds, cnn_type):
    ...
    # Only select land indexes
    choose = [0, 1, 2, 3, 4, 7, 8, 9, 12, 13, 14, 15, 16]
    print('Choose land indexes: {}'.format(np.array(indexes)[choose]))
    lbl = lbl[:, choose]
...
Fold 1: 0.0860362012675
Fold 2: 0.0878
Fold 3: 0.086705402459
Fold 4: 0.0843
Fold 5: 0.0844426939011
'''
