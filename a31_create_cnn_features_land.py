...
def get_validation_score_land(nfolds, cnn_type):
    global FOLD_TO_CALC
    ...
    choose = [0, 1, 2, 3, 4, 7, 8, 9, 12, 13, 14, 15, 16]
    ...
    for train_ids, valid_ids in kf.split(range(len(files))):
        ...
        cache_file = CACHE_PATH + '{}_valid_fold_{}_land.pklz'.format(cnn_type, num_fold)
        ...
        if not (os.path.isfile(cache_file) and restore_from_cache):
            final_model_path = MODELS_PATH + '{}_fold_{}_land.h5'.format(cnn_type, num_fold)
            print('Loading model {}...'.format(final_model_path))
            if cnn_type == 'RESNET101' or cnn_type == 'RESNET152' or 'DENSENET' in cnn_type:
                model = get_pretrained_model(cnn_type, len(choose), final_layer_activation='sigmoid')
    ...
    out = open(FEATURES_PATH + "valid_{}_score_{}_thr_{}_land.csv".format(cnn_type, best_score, best_thr), "w")
    # out = open(FEATURES_PATH + "valid_{}_land.csv".format(cnn_type), "w")
...
def process_test_land(nfolds, cnn_type, score, thr):
    global FOLD_TO_CALC
    from keras.models import load_model
    from keras import backend as K
    ...
    for num_fold in range(1, nfolds+1):
        ...
        cache_file = CACHE_PATH + '{}_test_fold_{}_land'.format(cnn_type, num_fold)
        print('Start KFold number {} from {}'.format(num_fold, nfolds))
    ...
    # Save raw file
    out = open(FEATURES_PATH + "test_{}_score_{}_thr_{}_land.csv".format(cnn_type, score, thr), "w")
    out.write("image_name")
    for i in choose:
        out.write("," + indexes[i])
    out.write("\n")
    ...
    # Create submission
    out = open(OUTPUT_PATH + "subm_{}_score_{}_thr_{}_land.csv".format(cnn_type, score, thr), "w")
...
if __name__ == '__main__':
    ...
    for cnn in ['DENSENET_121']:
        best_score, best_thr = get_validation_score_land(num_folds, cnn)
        process_test_land(num_folds, cnn, best_score, best_thr)


'''
Validation result:

Best overall score: 0.8783776613969391 THR: 0.19
Best score fold 1: 0.8717132327773861 THR: 0.21
Best score fold 2: 0.8810646984891833 THR: 0.22
Best score fold 3: 0.8828568139672164 THR: 0.19
Best score fold 4: 0.8775218783366753 THR: 0.21
Best score fold 5: 0.8796787233198925 THR: 0.19
'''